...
func TestComms(t *testing.T) {
	...
	{
		l2, err := GeneratePieceCIDFromFile(abi.RegisteredProof_StackedDRG2KiBPoSt, bytes.NewReader(make([]byte, 127)), 127)
		if err != nil {
			t.Fatal(err)
		}
		expPieceComms[0] = l2
	...
	for i := 1; i < levels-2; i++ {
		...
		expPieceComms[i], err = GeneratePieceCIDFromFile(abi.RegisteredProof_StackedDRG2KiBPoSt, r, sz)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestForSise(t *testing.T) {
	exp, err := GeneratePieceCIDFromFile(abi.RegisteredProof_StackedDRG2KiBPoSt, bytes.NewReader(make([]byte, 1016)), 1016)
	if err != nil {
		return
	}

	actual := ZeroPieceCommitment(1016)
